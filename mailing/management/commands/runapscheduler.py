...
def my_job():
    ...
    for mailing_setting in MailingSettings.objects.filter(status=MailingSettings.STATUS_STARTED):
        last_attempt = MailingAttempt.objects.filter(settings=mailing_setting).order_by('-attempt_date').first()

        if last_attempt:
            last_attempt_date = last_attempt.attempt_date
            time_difference = now - last_attempt_date

            if mailing_setting.period == MailingSettings.PERIOD_DAILY and time_difference.days >= 1:
            ...
            elif mailing_setting.period == MailingSettings.PERIOD_WEEKLY and time_difference.days >= 7:
            ...
            elif mailing_setting.period == MailingSettings.PERIOD_MONTHLY and time_difference.days >= 30:
